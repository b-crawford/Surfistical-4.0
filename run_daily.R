...
if(length(next_2)){
  rankings_url = paste('http://www.worldsurfleague.com/athletes/tour/wct?offset=51&sort=rank&year=', year, sep='')
  rankings = read_html(rankings_url)
  # Surfer names:
  name_indexes = html_nodes(rankings,'.athlete-name')
...
source('mens_predict_new_event.R')
source('womens_predict_new_event.R')
